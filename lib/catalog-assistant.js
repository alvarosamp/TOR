...
function fallbackReply(message, matches) {
    if (matches.length > 0) {
        ...
        return [
            ...
            'Voce pode abrir a pagina do item, conferir o datasheet revisado ou seguir para a solicitacao de compra.'
        ].filter(Boolean).join(' ');
    }
    ...
    return [
        'Nao encontrei um produto exato no catalogo revisado com essa descricao.',
...
function buildSystemPrompt(matches) {
    ...
    return [
        ...
        'O catalogo publico atual deve recomendar somente produtos com datasheet revisado disponiveis no contexto. Nao recomende QSFP ou 25G se eles nao estiverem no contexto recuperado.',
        ...
        'Explique o processo quando perguntarem: escolher o modelo, abrir a pagina do item, conferir especificacoes/datasheet revisado e enviar solicitacao de compra/cotacao pelo formulario.',
